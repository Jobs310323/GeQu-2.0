                            ...
                            <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2">
                                {sortedLogs.map((l: any) => (
                                    <div key={l.id ?? l.date} className="border-b border-[var(--border)] pb-4 anim-fade-in">
                                        ...
                                        {l.helped?.length > 0 && <div className="text-xs" style={{ color: 'var(--gq-good)' }}>Помогло: {l.helped.join(', ')}</div>}
                                        {l.hindered?.length > 0 && <div className="text-xs" style={{ color: 'var(--gq-bad)' }}>Мешало: {l.hindered.join(', ')}</div>}
                                    </div>
                                ))}
                            </div>
                        </div>
