...
import { COLOR_DOT_CLASS, COLOR_HEX, COLOR_OUTLINE_CLASS, MIND_COLORS, mindColorLabel, PRIORITY_HEX } from './colors';
    ...
    return (
        <div className="group relative">
            <div className={`absolute -top-10 left-1/2 -translate-x-1/2 flex items-center gap-1 glass-card rounded-full px-1.5 py-1.5 opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity ${selected ? 'opacity-100' : ''}`}>
                {MIND_COLORS.map(c => (
                    <button key={c} type="button" onClick={() => data.onRecolor(id, c)}
                        aria-label={t('plan:mindmap.node.colorLabel', { color: mindColorLabel(c, t) })} aria-pressed={c === data.color}
                ...
                <button type="button" onClick={() => data.onDelete(id)} aria-label={t('plan:mindmap.node.deleteLabel')}
            ...
            <div role="button" tabIndex={0}
                ...
                aria-label={t('plan:mindmap.node.openLabel', { text: data.text })}
            ...
            >
                <div className="flex items-center gap-2">
                    ...
                    {data.isMilestone && <span aria-label={t('plan:mindmap.node.milestoneLabel')} title={t('plan:mindmap.node.milestoneLabel')}>🏆</span>}
                    {editing ? (
                        <input
                            autoFocus
                            value={draft}
                        ...
                        <span onDoubleClick={e => { e.stopPropagation(); startEdit(); }} title={t('plan:mindmap.node.editHint')}
                            className="flex-1 text-sm text-[var(--text-main)] break-words">
                            {data.text}
                            {data.hasChildren && data.subtreeHours > 0 && (
                                <span className="text-[var(--text-muted)] font-normal">{t('plan:mindmap.node.subtreeHours', { hours: data.subtreeHours })}</span>
                            )}
                        </span>
                    )}
                    {data.tacticianMode && (
                        <button
                            ...
                            title={t('plan:mindmap.node.markDoneTitle')}
                ...
                {data.overdue && (
                    ...
                    >
                        <Icon name="clock" size={11} /> {t('plan:mindmap.node.snooze')}
                    </button>
                )}
            </div>
