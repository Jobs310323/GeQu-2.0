...
const TABS = [
    { id: 'progress', labelKey: 'insights:progress.tab.progress' },
    { id: 'dynamics', labelKey: 'insights:progress.tab.dynamics' },
    { id: 'snowman', labelKey: 'insights:progress.tab.snowman' },
];
...
export function Progress({ logs, habits, kanban, gymData, testResults, diary, snowmanDays }: ProgressProps) {
    ...
    return (
        <div className="max-w-5xl">
            <PageHeader page="progress" title={t('insights:progress.title')} />
                ...
                {TABS.map(item => (
                    <button key={item.id} onClick={() => setTab(item.id)}
                        className={`px-3.5 py-1.5 rounded-xl text-sm transition ${
                            tab === item.id ? 'bg-cyan-400/10 text-cyan-400' : 'text-[var(--text-muted)] hover:bg-white/5 hover:text-[var(--text-main)]'
                        }`}>
                        {t(item.labelKey)}
                    </button>
                ))}
                ...
                <>
                    ...
                    <div className="glass-card p-6 rounded-2xl mb-6 border border-cyan-400/25 bg-cyan-400/5">
                        <div className="flex flex-wrap items-end justify-between gap-4 mb-4">
                            <div>
                                <div className="text-xs uppercase tracking-wider text-gray-500 mb-1">{t('insights:progress.level')}</div>
                                <div className="text-5xl font-extrabold bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text text-transparent">
                                    {info.level}
                                </div>
                            </div>
                            <div className="text-right">
                                <div className="text-2xl font-bold text-white tabular-nums">{formatNumber(total)} XP</div>
                                <div className="text-xs text-gray-400">
                                    {t('insights:progress.toNextLevel', { level: info.level + 1, xp: formatNumber(xpToReach(info.level + 1) - total) })}
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="text-xs text-gray-500 tabular-nums">{t('insights:progress.intoLevel', { into: info.intoLevel, span: info.levelSpan })}</div>

                        <div className="mt-5 pt-4 border-t border-[var(--border)]">
                            <div className="text-cyan-400 font-bold mb-1">{t(chapter.titleKey)}</div>
                            <p className="text-sm text-gray-400">{t(chapter.textKey)}</p>
                        </div>
                    </div>

                    {/* Where XP came from */}
                    <div className="glass-card p-6 rounded-2xl mb-6">
                        <h2 className="text-xl font-bold mb-4">{t('insights:progress.xpSources')}</h2>
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                            {breakdown.map(b => (
                                <div key={b.key} className="bg-[var(--bg-input)] p-3 rounded-xl border border-[var(--border)]">
                                    <div className="flex items-center gap-2 mb-1">
                                        <span>{b.icon}</span>
                                        <span className="text-sm text-gray-300">{t(b.labelKey)}</span>
                                    </div>
                                    <div className="flex items-baseline justify-between">
                                        <span className="text-xs text-gray-500">{t('insights:progress.countUnit', { count: b.count })}</span>
                    ...
                    <div className="glass-card p-6 rounded-2xl">
                        <div className="flex items-center justify-between mb-4">
                            <h2 className="text-xl font-bold flex items-center gap-2">
                                ...
                                {t('insights:progress.achievements')}
                            </h2>
                            <span className="text-sm text-gray-400">{t('insights:progress.unlockedOf', { unlocked: unlockedCount, total: achievements.length })}</span>
                        </div>
                                ...
                                <div key={a.id}
                                    ...
                                    <div className="flex items-start gap-3">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className={`font-bold ${a.unlocked ? 'text-green-400' : 'text-gray-300'}`}>{t(a.titleKey)}</div>
                                            <div className="text-xs text-gray-500 mb-2">{t(a.descKey)}</div>
                                            {a.unlocked ? (
                                                <div className="text-[11px] text-green-400">{t('insights:progress.unlocked')}</div>
                                            ) : (
                                                <>
